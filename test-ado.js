...
async function testGetPrChanges() {
  ...
  // Import the function
  const { getPrFileChanges: getPrChanges } = await import('./dist/azureDevOps.js');

  try {
    const changes = await getPrChanges(TEST_PR_URL);
    console.log(`✓ Successfully fetched ${changes.length} file changes`);
    console.log("\nFirst 3 changes:");
    changes.slice(0, 3).forEach(change => {
      console.log(`  - ${change.filePath}`);
      console.log(`    Content lines: ${change.sourceContent.split('\n').length}`);
      console.log(`    Diff lines: ${change.patch.split('\n').length}`);
    });
  } catch (error) {
...
async function testPostPrComment() {
  ...
  // Import the function
  const { postPrComment } = await import('./dist/azureDevOps.js');
  ...
  try {
    ...
    const result = await postPrComment(options);
